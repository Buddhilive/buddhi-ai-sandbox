/**
 * Node.js crypto polyfill for the browser Web Worker environment
 */
import { Buffer } from './buffer.js';

// Fast pure JS SHA-256 implementation for synchronous createHash('sha256')
function sha256(ascii: Uint8Array): Uint8Array {
  ...
  for (i = 0; i < ascii[lengthProperty]; i++) {
    words[i >> 2] |= (ascii[i] & 0xff) << ((3 - (i % 4)) * 8);
  }
...
export class Hash {
  ...
  public digest(encoding?: string): any {
    ...
    const digestBytes = sha256(merged);
...
export class Hmac extends Hash {
  constructor(algorithm: string, key: any) {
    super(algorithm);
    this.update(key);
  }
}
